...
def create_user(user_in: UserCreate, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    user = UserModel(
        name=user_in.name,
        email=user_in.email,
...
def update_user_me(name: str = Form(None), email: EmailStr = Form(None), db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
    if name:
...
def list_products(q: str = None, min_price: float = None, max_price: float = None,
    ...
    if category:
        query = query.join(ProductModel.categories).filter(CategoryModel.name == category)
    products = query.all()
...
def create_product(item: ProductCreate, db: Session = Depends(get_db)):
    ...
    if item.category_ids:
        prod.categories = db.query(CategoryModel).filter(CategoryModel.id.in_(item.category_ids)).all()
    db.add(prod)
    db.commit()
    db.refresh(prod)
    return prod
...
def view_cart(db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
    ...
@app.post("/cart/items", response_model=Cart)
def add_to_cart(item: CartItemCreate, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
    ...
@app.delete("/cart/items/{product_id}", response_model=Cart)
def remove_from_cart(product_id: int, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
    ...

@app.post("/orders", response_model=Order)
def order_from_cart(db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
    ...

@app.get("/orders", response_model=list[Order])
def list_orders(db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
